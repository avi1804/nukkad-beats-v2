    ...
    <div className="min-h-screen bg-[#0B0B0F] text-text-light font-body pt-[100px]">
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12">
        ...
        >
          ...
          <h1 className="font-heading text-4xl font-bold text-white mb-2">Account Settings</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[280px] flex-shrink-0">
            <div className="bg-white/5 border border-white/10 rounded-[20px] p-4 backdrop-blur-md sticky top-[120px]">
              <nav className="flex flex-col gap-2">
                {tabs.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      <Icon size={18} className={isActive ? (tab.danger ? "text-red-400" : "text-gold") : "text-text-muted"} />
                      {tab.label}
                    </button>
                  );
                })}
...
function ProfileSection() {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[20px] p-8 backdrop-blur-md">
      <h2 className="text-2xl font-heading font-bold text-white mb-6">Profile Information</h2>
      
      <div className="flex items-center gap-6 mb-8 pb-8 border-b border-white/10">
        <div className="w-24 h-24 rounded-full bg-gold flex items-center justify-center font-heading text-4xl font-bold text-black shadow-[0_0_20px_rgba(216,154,43,0.3)]">
          {user?.fullName?.[0]?.toUpperCase() || "U"}
        </div>
        <div>
          <button className="px-4 py-2 rounded-xl bg-white/10 text-white font-medium text-sm border border-white/20 hover:bg-white/20 transition-colors">
            Change Avatar
          </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/50 transition-all"
            />
          </div>
          <div className="space-y-2">
        ...
        <div className="flex justify-end pt-4">
          <button 
            type="submit" 
            disabled={isLoading}
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gold text-black font-bold shadow-[0_8px_20px_rgba(216,154,43,0.2)] hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(216,154,43,0.3)] transition-all disabled:opacity-50 disabled:hover:translate-y-0"
          >
...
function SecuritySection() {
  ...
  if (isGoogleAuth) {
    return (
      <div className="bg-white/5 border border-white/10 rounded-[20px] p-8 backdrop-blur-md">
        <h2 className="text-2xl font-heading font-bold text-white mb-2">Security</h2>
        ...
        <div className="p-6 rounded-xl border border-blue-500/20 bg-blue-500/5 flex items-start gap-4">
          ...
          <div>
            <h3 className="text-white font-bold mb-1">Managed by Google</h3>
...
function PreferencesSection() {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[20px] p-8 backdrop-blur-md">
      <h2 className="text-2xl font-heading font-bold text-white mb-2">Notification Preferences</h2>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.02]">
          ...
          <button 
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${prefs.bookingEmails ? 'bg-gold' : 'bg-white/20'}`}
          >
            <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-black transition-transform ${prefs.bookingEmails ? 'translate-x-6' : 'translate-x-0'}`} />
          </button>
        </div>

        <div className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.02]">
          <div>
            <h4 className="text-white font-medium">WhatsApp Reminders</h4>
          ...
          <button 
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${prefs.whatsappReminders ? 'bg-gold' : 'bg-white/20'}`}
          >
            <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-black transition-transform ${prefs.whatsappReminders ? 'translate-x-6' : 'translate-x-0'}`} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.02]">
          ...
          <button 
            ...
            className={`w-12 h-6 rounded-full transition-colors relative ${prefs.promotionalEmails ? 'bg-gold' : 'bg-white/20'}`}
          >
            <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-black transition-transform ${prefs.promotionalEmails ? 'translate-x-6' : 'translate-x-0'}`} />
          </button>
        </div>
      </div>
    </div>
...
function SessionsSection() {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[20px] p-8 backdrop-blur-md">
      <h2 className="text-2xl font-heading font-bold text-white mb-2">Active Sessions</h2>
      ...
      <div className="space-y-4">
        {sessions.map(session => (
          <div key={session.id} className="flex items-center justify-between p-5 rounded-xl border border-white/10 bg-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-white font-medium flex items-center gap-2">
                  ...
                  {session.isCurrent && (
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-green-500/20 text-green-400 border border-green-500/20">Current</span>
                  )}
                </h4>
...
function DangerSection() {
  ...
  return (
    <div className="bg-red-500/5 border border-red-500/20 rounded-[20px] p-8 backdrop-blur-md">
      <h2 className="text-2xl font-heading font-bold text-red-500 mb-2">Danger Zone</h2>
      <p className="text-text-muted mb-8">Irreversible actions regarding your account.</p>

      <div className="space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-xl border border-white/5 bg-[#0B0B0F] gap-4">
          <div>
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 text-white font-medium border border-white/10 hover:bg-white/10 transition-colors"
          >
            <LogOut size={16} />
            Log out
          </button>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-xl border border-red-500/20 bg-red-500/10 gap-4">
          <div>
          ...
          {!showConfirm ? (
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-500/20 text-red-500 font-bold border border-red-500/30 hover:bg-red-500 hover:text-white transition-colors flex-shrink-0"
            >
              <Trash2 size={16} />
              Delete Account
            </button>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button 
                ...
                disabled={isDeleting}
                className="px-4 py-2.5 rounded-xl bg-white/5 text-text-muted hover:text-white transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button 
